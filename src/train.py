...
for epoch in range(epochs):
    ...
    loss = np.mean((y_pred.squeeze() - y_train) ** 2)

    dL_dy_pred = 2 * (y_pred.squeeze() - y_train) / len(y_train)  # pyright: ignore[reportUnknownVariableType, reportOperatorIssue]
...
print(y_pred_test[25], y_test[25])
